...
Tests the full metadata → VARIANT transformation chain across three layers:

Layer 1 — Real HTTP
  DICOMwebClient hits the public Orthanc demo and returns raw records.
  Connector yields metadata as JSON strings (StringType).

Layer 2 — Spark parse_json() transformation (no HTTP)
  _apply_column_expressions(df, {"metadata": "parse_json(metadata)"}) via
  selectExpr() produces a VariantType column — exactly what the SDP view does
  before apply_changes() in the Databricks pipeline.

Layer 3 — Full e2e: real HTTP + Spark DataFrame + VARIANT
  Simulates the complete pipeline: connector → DataFrame → parse_json() → VARIANT.
...
Layer 4 — OSS Declarative Pipeline (pyspark.pipelines)
  ...
  This pattern tests the same code path that runs in production without needing a
  full Spark Connect server.
...
class TestOrthanc:
    ...
    def test_reads_instances_with_metadata(self, orthanc_connector):
        ...
        for r in records_with_meta:
            # Connector must yield metadata as a plain JSON string (StringType)
            assert isinstance(r["metadata"], str), (
                f"Connector must yield metadata as str, got {type(r['metadata']).__name__}"
            )
...
class TestParseJsonTransformation:
    """Verify that selectExpr('parse_json(metadata) AS metadata') produces VariantType."""
...
# ---------------------------------------------------------------------------
# Layer 3 — Full end-to-end: real HTTP + Spark DataFrame + VARIANT
# ---------------------------------------------------------------------------
...
class TestEndToEndVariantPipeline:
    """
    Simulates what the Databricks pipeline does:

        lakeflow_connect readStream
          → _apply_column_expressions(df, {"metadata": "parse_json(metadata)"})   ← SDP view
          → apply_changes()                                                         ← Delta table

    Without the full streaming plumbing we replicate the same steps using a
    batch SparkSession and assert the resulting schema and data are correct.
    """

    def test_read_table_metadata_declares_column_expressions(self, orthanc_connector):
        meta = orthanc_connector.read_table_metadata("instances", {})
        assert "column_expressions" in meta, (
            "read_table_metadata('instances') must return column_expressions "
            "so the pipeline view can apply parse_json(metadata)"
        )
        assert meta["column_expressions"]["metadata"] == "parse_json(metadata)"

    @pytest.mark.skipif(not HAS_VARIANT, reason="pyspark >= 4.0 required for VariantType")
    def test_instances_metadata_string_before_transform(self, spark, orthanc_connector):
        """Connector emits metadata as StringType; schema matches INSTANCES_SCHEMA."""
        ...
        assert isinstance(meta_field.dataType, StringType), (
            "Before the pipeline transformation metadata must be StringType "
            "(connector emits JSON strings to avoid MALFORMED_VARIANT)"
        )

    def test_full_chain_metadata_becomes_variant(self, spark, orthanc_connector):
        """
        Full chain test:
          1. Connector reads real instances with WADO-RS metadata (JSON strings).
          2. Records are converted to a Spark DataFrame via INSTANCES_SCHEMA.
          3. _apply_column_expressions simulates the SDP view transform.
          4. Resulting DataFrame has metadata as VariantType.
          5. .collect() runs — no Spark errors on actual data.
        """
        from pyspark.sql.types import StringType, VariantType
        ...
        # Step 2: build DataFrame exactly as lakeflow_datasource.py does
        rows = [parse_value(r, INSTANCES_SCHEMA) for r in records]
        df = spark.createDataFrame(rows, INSTANCES_SCHEMA)

        meta_before = next(f for f in df.schema.fields if f.name == "metadata")
        assert isinstance(meta_before.dataType, StringType)

        # Step 3: apply the SDP view transformation
        result_df = _apply_column_expressions(df, column_expressions)

        # Step 4: schema check
        meta_after = next(f for f in result_df.schema.fields if f.name == "metadata")
        assert isinstance(meta_after.dataType, VariantType), (
            f"After parse_json() transformation 'metadata' must be VariantType, "
            f"got {meta_after.dataType}. The final Delta table will receive VARIANT."
        )

        # Step 5: data is collectable — no Spark evaluation errors
...
class TestDeclarativePipeline:
    ...
    append).  These are sufficient to test the parse_json(metadata) transformation.
    """
    ...
    def test_sdp_table_body_produces_variant_from_real_data(self, spark, orthanc_connector):
        ...
        This is the OSS equivalent of the Databricks SDP view + apply_changes() flow:
          @sdp.table   → full-replace table (vs apply_changes SCD merge on Databricks)
          parse_json() → same transformation, same VARIANT output

        Pipeline topology being tested:
          [Orthanc DICOMweb]
              ↓  (HTTP / QIDO-RS + WADO-RS)
          @sdp.temporary_view("instances_raw")     — connector records → STRING metadata DF
              ↓  (selectExpr parse_json)
          @sdp.table("instances")                  — VARIANT metadata Delta table
        """
        import pyspark.pipelines as sdp
        from pyspark.pipelines.graph_element_registry import graph_element_registration_context
        from pyspark.sql.types import StringType, VariantType
        ...
        with graph_element_registration_context(registry):
            ...
            @sdp.temporary_view(name="instances_raw")
            def instances_raw_fn():
                """Step 1: read from DICOMweb, build a STRING-metadata DataFrame."""
            ...
            def instances_fn():
                """Step 2: apply parse_json() — exactly the pipeline view transformation."""
                raw_df = spark.read.table("instances_raw")
                col_exprs = orthanc_connector.read_table_metadata("instances", {})["column_expressions"]
                return _apply_column_expressions(raw_df, col_exprs)

        # ---- Execute (simulate pipeline scheduler) ----------------------------
        # Step 1: run the temporary_view flow and register it as a Spark temp view
        ...
        assert isinstance(meta_final.dataType, VariantType), (
            f"@sdp.table('instances') must produce VariantType for metadata after "
            f"parse_json() transformation, got {meta_final.dataType}"
        )
    ...
    def test_sdp_append_flow_body_produces_variant(self, spark, orthanc_connector):
        ...
        with graph_element_registration_context(registry):
            ...
            def instances_append_fn():
                ...
                df = spark.createDataFrame(rows, INSTANCES_SCHEMA)
                col_exprs = orthanc_connector.read_table_metadata("instances", {})["column_expressions"]
                return _apply_column_expressions(df, col_exprs)
